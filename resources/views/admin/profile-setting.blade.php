...
<script>
    window.profileRoutes = {
        changePassword: "{{ route('profile.changePassword') }}",
        profilePic: "{{ route('profile.pic') }}",
        siteImage: "{{ route('site.image') }}"
    };
</script>

<script src="{{ asset('assets/js/api/profile-setting.ajax.js') }}"></script>
